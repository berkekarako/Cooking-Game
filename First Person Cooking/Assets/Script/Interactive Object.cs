...
public class InteractiveObject : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Input.GetKey(KeyCode.Q) && !_rb)
        {
            ...
            {
                if (raycastHit.collider.TryGetComponent(out KnifeTable knifeTable))
                {
                    knifeTable.Cut();
                }
            }
        ...
        if (Input.GetKeyDown(KeyCode.E))
        {
            if (_rb)
            {
                ...
                if (Physics.Raycast(ray, out var raycastHit, maxDis))
                {
                    ...
                    else if (raycastHit.collider.TryGetComponent(out KnifeTable knifeTable)) knifeTable.PlaceObj(_rb.gameObject, this);
                    else ObjectDrop();
                }
                else ObjectDrop();
            }
